...
import br.com.wellington.repository.UsuarioRepository;
...
public class UsuarioServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		Usuario usuario = usuarioRepository.findByLogin(username)
				.orElseThrow(() -> new UsernameNotFoundException("{campo.usuario.invalido}"));
		String[] roles = usuario.isAdmin() ? new String[] { "ADMIN", "USER" } : new String[] { "USER" };
		return User.builder().username(usuario.getLogin()).password(usuario.getSenha()).roles(roles).build();
		/*
		 * if (!username.equals("ciclano")) { throw new
		 * UsernameNotFoundException("Usuario não encontrado."); } return
		 * User.builder().username("ciclano").password(encoder.encode("123")).roles(
		 * "USER", "ADMIN").build();
		 */
	}
